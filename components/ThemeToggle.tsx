...
export function ThemeToggle({ className }: { className?: string }) {
  ...
  return (
    <button
      ...
      className={cn(
        "h-9 w-9 inline-flex items-center justify-center rounded-full border border-border text-muted-foreground hover:text-foreground hover:border-foreground/30 transition-colors",
        className,
      )}
      aria-label={isDark ? "Passer en mode clair" : "Passer en mode sombre"}
      title={isDark ? "Mode clair" : "Mode sombre"}
    >
      {mounted ? <SunMoonSwitch isDark={isDark} /> : null}
    </button>
  );
}

function SunMoonSwitch({ isDark }: { isDark: boolean }) {
  const duration = 0.7;
